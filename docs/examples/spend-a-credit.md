...
```
Settlement   0x1cD49739e9CF48C4906aDb44021dd8cE0d8aBa64   (SurplusSettlement, Base Sepolia)
...
the Tangle Router**, metered against your quota at the locked price:

```bash
curl -s https://surplus.178.104.232.124.sslip.io/redeem \
  -H 'content-type: application/json' \
  -d "{\"redemptionId\":\"$RID\",\"messages\":[{\"role\":\"user\",\"content\":\"Say hello\"}],\"maxTokens\":100}"
```
...
```bash
SIG=$(cast wallet sign --no-hash $RECEIPT_DIGEST --private-key $KEY)
curl -s https://surplus.178.104.232.124.sslip.io/redeem/receipt \
  -H 'content-type: application/json' \
  -d "{\"redemptionId\":\"$RID\",\"servedTokens\":$SERVED,\"signature\":\"$SIG\"}"
```
