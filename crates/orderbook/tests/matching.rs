...
fn rests_non_crossing_and_reports_top_of_book() {
    let mut b = book();
    b.place(order("b1", Side::Buy, 99_000, 5000, "u1", 1)).unwrap();
    b.place(order("a1", Side::Sell, 101_000, 5000, "u2", 2)).unwrap();
...
fn matches_at_maker_price_with_price_time_priority() {
    let mut b = book();
    b.place(order("a", Side::Sell, 100_000, 3000, "s", 1)).unwrap();
    b.place(order("b", Side::Sell, 100_000, 3000, "s", 2)).unwrap();
    b.place(order("c", Side::Sell, 99_000, 1000, "s", 3)).unwrap();

    let out = b.place(order("t", Side::Buy, 100_000, 5000, "buyer", 4)).unwrap();
    ...
    assert_eq!(prints, vec![("c", 1000, 99_000), ("a", 3000, 100_000), ("b", 1000, 100_000)]);
...
fn rests_unfilled_remainder() {
    let mut b = book();
    b.place(order("a", Side::Sell, 100_000, 2000, "s", 1)).unwrap();
    let out = b.place(order("t", Side::Buy, 100_000, 5000, "buyer", 2)).unwrap();
...
fn prevents_self_match_by_cancelling_resting_maker() {
    let mut b = book();
    b.place(order("mine", Side::Sell, 100_000, 2000, "mm", 1)).unwrap();
    let out = b.place(order("t", Side::Buy, 100_000, 2000, "mm", 2)).unwrap();
...
fn validates_tick_and_min_qty_and_duplicate_id() {
    ...
    b.place(order("dup", Side::Buy, 99_000, 2000, "u", 1)).unwrap();
    assert!(matches!(
...
fn cancel_and_depth_snapshot() {
    let mut b = book();
    b.place(order("x", Side::Buy, 99_000, 2000, "u", 1)).unwrap();
    b.place(order("y", Side::Buy, 99_000, 3000, "v", 2)).unwrap();
    b.place(order("z", Side::Buy, 98_000, 1000, "w", 3)).unwrap();
...
fn two_sided_market_seller_lists_buyer_lifts() {
    ...
    let mut b = book();
    b.place(order("seller", Side::Sell, 14_800_000, 100_000, "seller_op", 1)).unwrap();
    b.place(order("mm_bid", Side::Buy, 14_700_000, 50_000, "operator-mm", 2)).unwrap();
    let out = b
