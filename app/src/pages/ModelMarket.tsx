...
import { PriceChart as InstrumentChart } from '~/components/charts'
  ...
  return (
    <div>
      ...
      <div className="border-b border-[var(--s-border)] px-4 py-4 sm:px-6">
        <Link to="/" className="mb-3 inline-flex items-center gap-1.5 font-data text-[13px] text-[var(--s-text-muted)] hover:text-[var(--s-text-secondary)]">
      ...
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:px-6 lg:grid-cols-12">
        ...
        <Panel
          ...
          right={<span className="font-data text-[12px] text-[var(--s-text-muted)]">{kind}</span>}
        >
      ...
      <div className="px-4 pb-8 sm:px-6">
        <Panel
          ...
          right={<span className="font-data text-[12px] text-[var(--s-text-muted)]">{offers.length} live</span>}
        >
                    ...
                    <tr key={o.id} className="border-b border-[var(--s-divider)] last:border-0 hover:bg-[var(--s-panel)]">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          {o.verified && <span className="i-ph:seal-check text-[14px] text-[var(--s-accent)]" title="Verified supply" />}
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-right font-data text-[13px] tabular-nums text-[var(--s-emerald)]">{pct(o.discount, 0)}</td>
                      <td className="px-3 py-2.5 text-right font-data text-[13px] font-semibold tabular-nums text-[var(--s-text)]">{pricePerM(o.price[kind])}</td>
                      <td className="px-3 py-2.5 text-right font-data text-[13px] tabular-nums text-[var(--s-text-muted)]">{tokens(o.offeredTokens)}</td>
                      <td className="px-3 py-2.5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <span className="font-data text-[13px] tabular-nums text-[var(--s-text-muted)]">{tokens(o.soldTokens)}</span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-right font-data text-[13px] tabular-nums text-[var(--s-text-secondary)]">{tokens(o.remainingTokens)}</td>
                      <td className="px-3 py-2.5 text-right font-data text-[12px] tabular-nums text-[var(--s-text-subtle)]">{timeAgo(Date.now() - o.ageS * 1000)}</td>
                      <td className="px-3 py-2.5 text-right">
                        <button onClick={() => navigate(`/buy/${model.id}`)} className="btn-primary h-6 !px-2.5 !text-[11px]">
                          Buy
                        </button>
                      </td>
                    </tr>
...
/** Real instrument chart: gradient market area vs dashed list reference. */
function PriceChart({ points, list }: { points: number[]; list: number }) {
  return <InstrumentChart points={points} listPrice={list} height={220} formatY={pricePerM} />
}
