//! InferenceBazaar operator library.
//!
//! The venue (open orderbook + sidecar quoting + settlement intents) lives here
//! so BOTH bins drive the same market:
//!   - `inference-bazaar-operator-lite` (src/main.rs): HTTP venue only, no Tangle substrate.
//!   - `inference-bazaar-operator`      (src/bin/blueprint.rs, feature `blueprint`): the
